...
export function registerOAuthRoutes(app: Express) {
  // Mock OAuth server endpoints for local development
  app.get("/app-auth", (req: Request, res: Response) => {
    console.log("[OAuth] /app-auth called", req.query);
    const redirectUri = getQueryParam(req, "redirectUri");
    const state = getQueryParam(req, "state");

    if (!redirectUri || !state) {
      console.error("[OAuth] Missing redirectUri or state");
  ...
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log("[OAuth] Setting cookie:", COOKIE_NAME, cookieOptions);
